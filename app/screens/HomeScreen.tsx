...
export default function HomeScreen() {
  ...
  function getNextDayId() {
    const daysWithLogs = data.days
      .map((d) => {
        ...
        return {
          dayId: d.id,
          lastDate: lastLog ? new Date(lastLog.date) : null,
        };
      })
      .sort(
        (a, b) =>
    ...
    const lastDayIndex = data.days.findIndex(
      (d) => d.id === daysWithLogs[0].dayId,
    );
  ...
  function getDayProgress(day: any) {
    ...
    const completed = day.workouts.filter((w: any) =>
      w.logs?.some((l: any) => l.date === today),
    ).length;
  ...
  return (
    <SafeAreaView
      style={{ flex: 1, backgroundColor: isDayMode ? "#f0f4f8" : "#1e1e2f" }}
    >
      <LinearGradient
      ...
      >
        ...
        <View style={styles.header}>
          ...
          <TouchableOpacity onPress={() => setSelectedDayId(null)}>
            <Text
              style={[styles.title, { color: isDayMode ? "#222" : "#fff" }]}
            >
              GymBro
            </Text>
          </TouchableOpacity>
        ...
        {!selectedDayId ? (
          <ScrollView contentContainerStyle={styles.daysContainer}>
            <Text
              style={[styles.greeting, { color: isDayMode ? "#222" : "#fff" }]}
            >
              ...
              return (
                ...
                >
                  <LinearGradient
                    ...
                    style={[
                      styles.dayCard,
                      nextDayId === d.id ? styles.dayCardNext : {},
                    ]}
                  >
                    <Text style={styles.dayText}>
                      {getDayIcon(d.name)} {d.name}
                    </Text>
                    <Text style={styles.workoutCount}>
                      {d.workouts.length} workouts
                    </Text>
                    ...
                    {nextDayId === d.id && (
                      <Text style={styles.nextBadge}>Next Up</Text>
                    )}
                  </LinearGradient>
                </TouchableOpacity>
              );
          ...
          <View style={{ flex: 1, paddingHorizontal: 16 }}>
            ...
            <FlatList
              ...
              ListEmptyComponent={
                <Text
                  style={{ padding: 12, color: isDayMode ? "#222" : "#fff" }}
                >
                  No workouts yet
                </Text>
              }
